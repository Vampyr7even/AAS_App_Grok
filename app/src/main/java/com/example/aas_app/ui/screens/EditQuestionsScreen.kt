...
@Composable
fun EditQuestionsScreen(navController: NavController) {
    val viewModel: AdminViewModel = hiltViewModel()
    val questionsState by viewModel.questionsState.observeAsState(AppState.Loading<List<PeclQuestionEntity>>())

    LaunchedEffect(Unit) {
        viewModel.loadAllQuestions() // Assume added to ViewModel/Repo: getAllQuestions
    }
    ...
    Column(
        ...
        Button(
            onClick = { viewModel.insertQuestion(PeclQuestionEntity(0L, newSubTask, "Text", "Scale", "No"), 0L) },
