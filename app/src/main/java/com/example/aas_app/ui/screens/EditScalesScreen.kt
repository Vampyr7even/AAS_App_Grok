...
fun EditScalesScreen(navController: NavController) {
    ...
    var selectedScale by remember { mutableStateOf<PeclScaleEntity?>(null) }
    ...
    Column(
        ...
        Button(
            onClick = { viewModel.insertScale(PeclScaleEntity(0L, newScaleName, "")) }, // Assume insertScale added
    ...
    if (showDialog) {
        AlertDialog(
            ...
            confirmButton = {
                Button(onClick = {
                    selectedScale?.let { viewModel.deleteScale(it) } // Assume deleteScale added
                    showDialog = false
                }) {
                    Text("Yes")
                }
